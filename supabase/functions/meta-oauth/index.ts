...
async function processMetaOAuth(params: { code: string; statePayload: string; redirectUriOverride?: string }): Promise<OAuthProcessResult> {
  ...
  const tokenResponse = await fetch(`https://graph.facebook.com/v19.0/oauth/access_token?client_id=${META_CLIENT_ID}&redirect_uri=${encodeURIComponent(effectiveRedirectUri)}&client_secret=${META_CLIENT_SECRET}&code=${code}`);
  ...
  if (sourcePlatform === 'instagram' && !grantedPermissions.has('instagram_content_publish')) {
    throw new Error("Missing instagram_content_publish permission. Please reconnect and approve Instagram publishing permission.");
  }
  ...
  for (const page of pagesData.data) {
    ...
    if (igAccountId) {
      if (sourcePlatform === 'instagram') {
        await verifyInstagramPublishPermission(igAccountId, page.access_token);
      }
