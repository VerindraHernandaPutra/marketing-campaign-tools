...
serve(async (req) => {
  // 1. Webhook verification — Meta sends GET when you register/save the webhook URL
  if (req.method === 'GET') {
  ...
  // 2. Incoming events from Meta — always return 200 so Meta stops retrying on errors
  if (req.method === 'POST') {
    ...
    try {
      ...
      for (const entry of body.entry || []) {
        for (const change of entry.changes || []) {
          ...
          // Find the organization that owns this Phone Number ID
          ...
          // Build contact name map from the contacts array
      ...
      // Log but still return 200 — never let Meta think the endpoint is down
