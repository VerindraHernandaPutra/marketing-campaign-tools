...
const AdminDashboard: React.FC = () => {
  ...
  const openUserModal = (mode: 'create' | 'edit', user?: ProfileWithRole) => {
    ...
    if (mode === 'edit' && user) {
        ...
        setUserFormEmail('');
        setUserFormPassword('');
        setUserFormOrgId(null);
        setUserFormRole('designer');
        ...
        if (userModalMode === 'create') {
            ...
            if (selectedUser?.membership_id) {
                const { error } = await supabase.from('organization_members').update({ 
                    role: userFormRole, 
                    status: statusValue,
                    organization_id: userFormOrgId 
                }).eq('id', selectedUser.membership_id);
                if(error) throw error;
                            ...
                            <Table.Tr key={org.id} style={{ opacity: org.status === 'inactive' ? 0.6 : 1 }}>
                                ...
                                <Table.Td>
                                    <Group gap="xs">
                                        ...
                                        <Tooltip label={org.status === 'active' ? 'Deactivate' : 'Activate'}><ActionIcon variant="default" color={org.status === 'active' ? 'orange' : 'green'} onClick={() => toggleOrgStatus(org.id, org.status || 'active')}>{org.status === 'active' ? <BanIcon size={16} /> : <CheckCircleIcon size={16} />}</ActionIcon></Tooltip>
                    ...
                    <Table.Tbody>
                        {processUsers.data.map(u => (
                            <Table.Tr key={u.id}>
                                ...
                                <Table.Td>{u.role !== '-' && <Badge variant="light" color={u.role === 'admin' ? 'red' : u.role === 'operator' ? 'blue' : u.role === 'designer' ? 'pink' : 'cyan'}>{u.role?.toUpperCase()}</Badge>}</Table.Td>
                                ...
                                <Table.Td>
                                    <Group gap="xs">
                                        ...
                                        <Tooltip label={u.status === 'active' ? 'Deactivate' : 'Activate'}><ActionIcon variant="default" color={u.status === 'active' ? 'orange' : 'green'} disabled={!u.membership_id} onClick={() => toggleUserStatus(u.membership_id, u.status || 'active')}>{u.status === 'active' ? <BanIcon size={16} /> : <CheckCircleIcon size={16} />}</ActionIcon></Tooltip>
      ...
      {/* --- ORG MODAL --- */}
        <Modal opened={isOrgModalOpen} onClose={() => setIsOrgModalOpen(false)} title={`${orgModalMode === 'create' ? 'Create' : 'Edit'} Organization`} size="lg">
          ...
          <Group justify="space-between" mb="xl">
            ...
            <Switch checked={orgFormStatus} onChange={(event) => setOrgFormStatus(event.currentTarget.checked)} label={orgFormStatus ? "Active" : "Inactive"} color="green" />
          </Group>
        ...
        {/* --- USER MODAL --- */}
        <Modal opened={isUserModalOpen} onClose={() => setIsUserModalOpen(false)} title={`${userModalMode === 'create' ? 'Add New' : 'Edit'} User`}>
            ...
            {userModalMode === 'create' && <PasswordInput label="Password" placeholder="Initial password" mb="sm" value={userFormPassword} onChange={e => setUserFormPassword(e.target.value)} required />}
            ...
            <Select label="Role" data={[{ value: 'operator', label: 'Operator (Admin)' }, { value: 'designer', label: 'Designer' }, { value: 'marketer', label: 'Marketer' }]} value={userFormRole} onChange={(val) => setUserFormRole(val || 'designer')} allowDeselect={false} mb="xl" />
            <Group justify="space-between" mb="xl">
                <Text size="sm" fw={500}>Status</Text>
                <Switch checked={userFormStatus} onChange={(event) => setUserFormStatus(event.currentTarget.checked)} label={userFormStatus ? "Active" : "Inactive"} color="green" />
            </Group>
