...
export default function Inbox() {
  ...
  // Ref keeps the latest activeConvo accessible inside realtime callbacks without stale closure
  ...
  // Single global subscription — avoids stale closure on activeConvo
  useEffect(() => {
    if (!currentOrgId) return;
    fetchConversations();
    ...
    const msgSub = supabase
      ...
      .on('postgres_changes', {
        ...
        // Append to active conversation without duplicating
    ...
    try {
      ...
      const res = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-chat-message`,
        {
          ...
          body: JSON.stringify({ conversationId: activeConvo.id, content: inputText.trim(), mediaUrl: uploadedMediaUrl }),
        }
      );
                    ...
                    >
                      <Group wrap="nowrap" gap="sm">
                        ...
                        <Box style={{ flex: 1, overflow: 'hidden', minWidth: 0 }}>
                          ...
                          {/* Phone number for WhatsApp when name is set */}
