...
import { Container, Title, Table, Button, Group, Modal, Select, Loader, Badge, ActionIcon, Text, Paper, Breadcrumbs, Anchor, Tabs, TextInput, PasswordInput, Tooltip, Switch, SimpleGrid, RingProgress, Center } from '@mantine/core';
...
const OrganizationDetails: React.FC = () => {
  ...
  const fetchOrgDetails = useCallback(async () => {
    ...
    if (org) {
        // Safe casting now that we know the columns exist (after migration)
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        // --- Limit Check (Frontend Pre-check) ---
        // (Note: Database trigger should also enforce this)
        if (formStatus && orgData) {
            ...
            // If adding new active user OR editing inactive user to active
            // We need to check if limit reached.
            // Simplified check:
  ...
  // Helper for RingProgress
  const StatRing = ({ label, count, limit, color }: { label: string, count: number, limit: number, color: string }) => {
        ...
        <Paper withBorder p="xs" radius="md">
            <Group>
                <RingProgress
                    ...
                    label={
                        <Center>
                            <Text fz="xs" fw={700} ta="center">{Math.round(percentage)}%</Text>
                        </Center>
                    }
                />
                <div>
                    ...
                    <Text fw={700} size="md">
                        {count} <span style={{fontSize: '0.8em', color: '#999'}}>/ {limit}</span>
                    </Text>
                </div>
            </Group>
        </Paper>
      ...
      <div className="w-full min-h-screen bg-white dark:bg-gray-900 flex flex-col">
        ...
        <div className="flex flex-1">
          ...
          <Container size="xl" py="xl" className="flex-1">
            ...
            <Group justify="space-between" mb="lg">
                ...
                <Button leftSection={<PlusIcon size={16}/>} onClick={() => handleOpenModal('add')}>
                    Add User
                </Button>
            </Group>
                ...
                {loading ? <Loader /> : (
                    <Table striped highlightOnHover>
                        ...
                        <Table.Tbody>
                                ...
                                <Table.Tr key={m.id} style={{ opacity: m.status === 'inactive' ? 0.5 : 1 }}>
                                    ...
                                    <Table.Td>
                                        <Badge variant="light" color={
                                            m.role === 'admin' ? 'red' : 
                                            m.role === 'operator' ? 'blue' : 
                                            m.role === 'designer' ? 'pink' : 'cyan'
                                        }>{m.role.toUpperCase()}</Badge>
                                    </Table.Td>
                                    <Table.Td>
                                        <Badge size="sm" color={m.status === 'active' ? 'green' : 'gray'} variant="dot">
                                            {m.status}
                                        </Badge>
                                    </Table.Td>
                                    <Table.Td>
                                        <Group gap="xs">
                                            <Tooltip label="Edit User">
                                                <ActionIcon variant="default" onClick={() => handleOpenModal('edit', m)}>
                                                    <EditIcon size={16} />
                                                </ActionIcon>
                                            </Tooltip>
                                            ...
                                            <Tooltip label={m.status === 'active' ? 'Deactivate User' : 'Activate User'}>
                                                <ActionIcon 
                                                    variant="default" 
                                                    color={m.status === 'active' ? 'orange' : 'green'}
                                                    onClick={() => toggleUserStatus(m.id, m.status || 'active')}
                                                >
                                            ...
                                            <Tooltip label="Remove User">
                                                <ActionIcon variant="light" color="red" onClick={() => handleDelete(m.id)}>
                                                    <TrashIcon size={16} />
                                                </ActionIcon>
                                            </Tooltip>
                                        </Group>
                            ...
                            {members.length === 0 && <Table.Tr><Table.Td colSpan={5} align="center">No members found</Table.Td></Table.Tr>}
                        </Table.Tbody>
                    </Table>
                )}
            </Paper>
        ...
        >
                ...
                <Tabs value={addMethod} onChange={(val) => setAddMethod(val as 'existing' | 'new')} mb="md">
                    ...
                    <Tabs.Panel value="existing" pt="xs">
                        <Select 
                            ...
                            searchable
                            nothingFoundMessage="No available users found"
                            mb="md"
                        />
                    </Tabs.Panel>
            ...
            <Select 
                label="Role"
                data={[
                    { value: 'operator', label: 'Operator (Admin)' },
                    { value: 'designer', label: 'Designer' },
                    { value: 'marketer', label: 'Marketer' }
                ]}
                value={formRole}
                onChange={(val) => setFormRole(val || 'designer')}
                allowDeselect={false}
                mb="md"
            />
            ...
            <Group justify="space-between" mb="xl">
                ...
                <Switch checked={formStatus} onChange={(event) => setFormStatus(event.currentTarget.checked)} label={formStatus ? "Active" : "Inactive"} color="green" />
            </Group>
