...
import { 
  ...
  BoxIcon, HexagonIcon, MinusIcon, BaselineIcon, SparklesIcon, Wand2Icon, 
  Settings2Icon, InfoIcon, PaletteIcon, CameraIcon, SunIcon, MaximizeIcon, ScalingIcon
} from 'lucide-react';
import { useFabricCanvas } from '../../context/CanvasContext';
// FIX: Ensure this path is correct relative to this file
...
const Sidebar: React.FC<SidebarProps> = () => {
  ...
  const addText = (textType: 'heading' | 'subheading' | 'paragraph') => {
    ...
    switch (textType) {
      case 'heading': text = new Textbox('Heading', { left: 100, top: 100, fontFamily: 'Arial', fill: '#000000', fontSize: 48, fontWeight: 'bold' }); break;
      case 'subheading': text = new Textbox('Subheading', { left: 100, top: 100, fontFamily: 'Arial', fill: '#000000', fontSize: 32, fontWeight: 'normal' }); break;
      case 'paragraph': text = new Textbox('Paragraph text', { left: 100, top: 100, fontFamily: 'Arial', fill: '#000000', fontSize: 16, width: 250 }); break;
    }
    if (text) {
      canvas.add(text);
    ...
    input.onchange = (e) => {
      ...
      reader.onload = async (f) => {
        try {
          ...
          notify.error('Import Failed', 'Could not load the selected image.');
        }
      };
  ...
  const handleGenerateAI = async () => {
    ...
    try {
        ...
        if (data && data.image) {
            ...
            canvas.setActiveObject(img);
            canvas.renderAll();
            notify.success('Image Generated', 'AI image has been added to canvas.');
        }
    } catch (error: unknown) {
      ...
      <ScrollArea h="calc(100vh - 140px)" mx="-xs" px="xs">
        {activeTab === 'elements' ? <Accordion multiple defaultValue={['ai']}>
              ...
              <Accordion.Panel>
                <Stack gap="xs">
                  <SegmentedControl
                    ...
                    data={[
                      { label: (<Group gap={6}><Wand2Icon size={14} /><Text size="xs">Basic</Text></Group>), value: 'basic' },
                      { label: (<Group gap={6}><Settings2Icon size={14} /><Text size="xs">Advanced</Text></Group>), value: 'advanced' },
                    ]}
                    size="xs"
                    fullWidth
                    mb="xs"
                  ...
                  <Select
                    ...
                    data={['Photorealistic', 'Cinematic', '3D Render', 'Anime', 'Digital Art', 'Oil Painting', 'Watercolor', 'Cyberpunk', 'Minimalist', 'Sketch', 'Vintage', 'Pop Art', 'Isometric']}
                  ...
                  <Collapse in={promptMode === 'advanced'}>
                    <Stack gap="xs" mt="xs">
                      ...
                      <SimpleGrid cols={2}>
                        <div>
                          <Group gap={4} mb={2}><SunIcon size={12} style={{ opacity: 0.7 }} /><Text size="xs" fw={500} c="dimmed">Lighting</Text></Group>
                          <Select placeholder="Select" data={['Natural', 'Studio', 'Neon', 'Golden Hour', 'Cinematic', 'Dark/Moody', 'Softbox']} value={aiParams.lighting} onChange={(val) => setAiParams({...aiParams, lighting: val || ''})} size="xs" />
                        </div>
                        <div>
                          <Group gap={4} mb={2}><PaletteIcon size={12} style={{ opacity: 0.7 }} /><Text size="xs" fw={500} c="dimmed">Colors</Text></Group>
                          <Select placeholder="Select" data={['Vibrant', 'Pastel', 'Black & White', 'Warm Tone', 'Cool Tone', 'Muted', 'Neon']} value={aiParams.colorPalette} onChange={(val) => setAiParams({...aiParams, colorPalette: val || ''})} size="xs" />
                        </div>
                      </SimpleGrid>

                      <SimpleGrid cols={2}>
                        <div>
                          <Group gap={4} mb={2}><CameraIcon size={12} style={{ opacity: 0.7 }} /><Text size="xs" fw={500} c="dimmed">Angle</Text></Group>
                          <Select placeholder="Select" data={['Wide Shot', 'Close Up', 'Macro', 'Drone View', 'Eye Level', 'Low Angle']} value={aiParams.composition} onChange={(val) => setAiParams({...aiParams, composition: val || ''})} size="xs" />
                        </div>
                        <div>
                          <Group gap={4} mb={2}><BoxIcon size={12} style={{ opacity: 0.7 }} /><Text size="xs" fw={500} c="dimmed">Texture</Text></Group>
                          <Select placeholder="Select" data={['Smooth', 'Rough', 'Metallic', 'Wooden', 'Glass', 'Fabric', 'Paper']} value={aiParams.texture} onChange={(val) => setAiParams({...aiParams, texture: val || ''})} size="xs" />
                        </div>
                      </SimpleGrid>

                      <Divider label="Context" labelPosition="center" mt="xs" />
                      <Text size="xs" fw={500} c="dimmed">Background</Text>
                      <TextInput placeholder="e.g. Busy street, White studio, Forest" value={aiParams.background} onChange={(e) => setAiParams({...aiParams, background: e.currentTarget.value})} size="xs" />

                      <SimpleGrid cols={2}>
                          <div>
                            <Text size="xs" fw={500} c="dimmed">Mood</Text>
                            <Select placeholder="Select" data={['Happy', 'Professional', 'Dark', 'Energetic', 'Calm', 'Mysterious', 'Romantic']} value={aiParams.mood} onChange={(val) => setAiParams({...aiParams, mood: val || ''})} size="xs" />
                          </div>
                          <div>
                            <Text size="xs" fw={500} c="dimmed">Purpose</Text>
                            <Select placeholder="Select" data={['Social Media', 'Ad Banner', 'Logo', 'Website', 'Wallpaper']} value={aiParams.purpose} onChange={(val) => setAiParams({...aiParams, purpose: val || ''})} size="xs" />
                          </div>
                      </SimpleGrid>
                      ...
                      <SimpleGrid cols={2}>
                        <div>
                           <Group gap={4} mb={2}><MaximizeIcon size={12} style={{ opacity: 0.7 }} /><Text size="xs" fw={500} c="dimmed">Aspect Ratio</Text></Group>
                          <Select value={aiParams.aspectRatio} onChange={(val) => setAiParams({...aiParams, aspectRatio: val || 'square'})} data={[{ value: 'square', label: 'Square (1:1)' }, { value: 'landscape', label: 'Landscape (16:9)' }, { value: 'portrait', label: 'Portrait (9:16)' }, { value: 'custom', label: 'Custom Size' }]} size="xs" allowDeselect={false} />
                        </div>
                        <div>
                           <Group gap={4} mb={2}><SparklesIcon size={12} style={{ opacity: 0.7 }} /><Text size="xs" fw={500} c="dimmed">Quality</Text></Group>
                          <Select value={aiParams.quality} onChange={(val) => setAiParams({...aiParams, quality: val || 'standard'})} data={[{ value: 'standard', label: 'Standard' }, { value: 'hd', label: 'High Def (HD)' }]} size="xs" allowDeselect={false} />
                        </div>
                      </SimpleGrid>
                      ...
                      <Collapse in={aiParams.aspectRatio === 'custom'}>
                        <SimpleGrid cols={2} mt="xs">
                            <div>
                                <Group gap={4} mb={2}><ScalingIcon size={12} style={{ opacity: 0.7 }} /><Text size="xs" fw={500} c="dimmed">Width (px)</Text></Group>
                                <NumberInput value={aiParams.width} onChange={(val) => setAiParams({...aiParams, width: Number(val) || 1024})} size="xs" min={256} max={2048} />
                            </div>
                            <div>
                                <Group gap={4} mb={2}><ScalingIcon size={12} style={{ opacity: 0.7 }} /><Text size="xs" fw={500} c="dimmed">Height (px)</Text></Group>
                                <NumberInput value={aiParams.height} onChange={(val) => setAiParams({...aiParams, height: Number(val) || 1024})} size="xs" min={256} max={2048} />
                            </div>
                        </SimpleGrid>
                      </Collapse>
                      ...
                      <div>
                        ...
                        <Textarea placeholder="e.g. blurry, low quality, text, distorted" autosize minRows={2} value={aiParams.negativePrompt} onChange={(e) => setAiParams({...aiParams, negativePrompt: e.currentTarget.value})} size="xs" />
                      </div>
                    </Stack>
                  </Collapse>
          ...
          <SimpleGrid cols={2} spacing="md">
            {Array(8).fill(0).map((_, i) => <div key={i} style={{ height: 120, backgroundColor: `light-dark(${theme.colors.gray[2]}, ${theme.colors.dark[6]})`, borderRadius: theme.radius.md, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Text size="xs" color="dimmed">Template {i + 1}</Text>
              </div>)}
          </SimpleGrid>}
      </ScrollArea>
    </Box>
